...
!      type (type_diagnostic_variable_id)   :: id_bbp450, id_bbp550, id_bbp700
      ...
      type (type_diagnostic_variable_id)   :: id_NAP, id_CDOM
...
!     call self%register_dependency(self%id_NAP,NAP)
...
!     call self%register_dependency(self%id_CDOM,CDOM)
...
!     call self%add_to_aggregate_variable(CDOM,     self%id_X3c,  include_background=.true.)
      ...
      call self%register_diagnostic_variable(self%id_NAP, 'NAP', 'mg C/m^3', 'NAP', source=source_do_column)
      call self%register_diagnostic_variable(self%id_CDOM, 'CDOM', 'mg C/m^3', 'CDOM', source=source_do_column)
...
!      real(rk) :: equis(self%nlt),ies(self%nlt)
      ...
      real(rk) :: NAP(cache%n),CDOM(cache%n)
...
!              tot_a  =  aw(l) + phy_a  + apoc(l) * (R6c + R8c) + cdom_a  + spm_a
             tot_b  =  bw(l) + phy_b  + bpoc(l) * R6c  + spm_b
!              tot_b  =  bw(l) + phy_b  + bpoc(l) * (R6c + R8c)  + spm_b
             tot_bb = bbw(l) + phy_bb + bbpoc(l)* R6c  + spm_bb
!              tot_bb = bbw(l) + phy_bb + bbpoc(l)* (R6c + R8c)  + spm_bb
...
!     _HORIZONTAL_LOOP_END_
      ...
      _DOWNWARD_LOOP_BEGIN_
         ...
         _SET_DIAGNOSTIC_(self%id_NAP, NAP(kk))          
         _SET_DIAGNOSTIC_(self%id_CDOM, CDOM(kk))         
     _DOWNWARD_LOOP_END_
